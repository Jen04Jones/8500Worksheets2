...
locations <- data %>% select(city ,state) 
...
I first created a function called locations and I assigned it to take from the gayguide just city and state from the gayguides dataset to make
a dataframe.
I then specified it was from the dataset gayguides and used the verb select () to call the location function and print it. 
I added eval: false to limit this massive database.
>
...
data("gayguides")
 ...
 eval: false
#list_names <- colnames (gayguides [colnames(gayguides)])
#list_names

gayguides_selected <- gayguides %>% 
  select(-city, -state)


print(gayguides_selected)
...
locnysan75 <- gayguides %>%
  filter(Year == 1975 & state != "New York" & state != "San Francisco")
...
filtered_gayguides <- gayguides %>%
  filter(!is.na(amenityfeatures))
         (grepl("G", amenityfeatures) | grepl("L", amenityfeatures))

print(filtered_gayguides)
...
colnames(statepopulations)

pop1850 <-statepopulations %>%
  select(STATE, `X1850`) %>% 
  arrange(desc(`X1850`))     
...
south_carolina_loc <- gayguides %>%
  filter(state == "SC" & Year >= 1970 & Year <= 1975)

  city_counts <- south_carolina_loc %>%
  group_by(city) %>%
  summarize(locations = n())

average_locations <- mean(city_counts$locations, na.rm = TRUE)
...
southernstates <- c("AL", "AR", "FL", "GA", "KY", "LA", "MD", "MS", "NC",
                    "SC", "TN", "TX", "VI", "WV")


    states_loc <- gayguides %>%
    filter(state %in% southernstates & Year == 1975)


       state_counts <- states_loc %>%
       group_by(state) %>%
        summarize(count = n())

       total_loc <- nrow(states_loc)
...
gayguides_joined <- gayguides %>%
  left_join(regions, by = c("state" = "State.Code"))

  #test
  summary(gayguides_joined)
  head(gayguides_joined)

mountain_total <- gayguides_joined %>%
  filter(Division == "Mountain", Year == 1980) %>%
  group_by(Division) %>%
  summarize(total_locations = n())

print(mountain_total)

```

(@) Explain what you did above. What variable did you join by and why? What results?

I was having a lot of trouble with this, it kept returning integer 0. I've tried a bunch of little tests
 to see why things aren't matching up. I found that I needed to make sure everything matched up. 
 So I tested the full gayguides_joined (gaygudies+region) After a lot of trial and error I figured out
 that I had to use State.Code and Division because these gave better connections. I had started with
 using State for Regions, and Region for Division. But finally figured out how these two datasets
 matched up and how to filter, for year and Mountain. 
...
LGTBQ_growth <- gayguides_joined %>%
  filter(Year %in% c(1970, 1980)) %>% 
  group_by(Year) %>%
  summarize(total_locations = n())


LGTBQ_growth <- LGTBQ_growth  %>%
...
The pivot_longer() fuction changes the dataset from in 'wide format' to 'long format' by
restructuring the data.
It made a new dataset that each row corresponds to city, worker type, year, and count of workers.
        city  type_of_worker X1930 X1935 X1940 X1945
1 Charleston       male paid     7     9    NA    NA
2 Charleston     female paid     7     8    NA    NA
3 Charleston   men volunteer     4    10    NA    NA
4 Charleston women volunteer     4     5    NA    NA
5   Columbia       male paid     6    NA    NA     4
6   Columbia     female paid    16    NA    NA    42
...
pivot_longer(!type, names_to = "space_type", values_to = "count")


print(rec.spaces_long)
...
pivot_wider(!type_of_worker = "year", values_to = "count")
...
rec.spaces_wide <- rec.spaces %>%
pivot_wider(!type, names_to = "space_type", values_to = "count")
...
(@) First, tell me, what is the question you are asking?
 Question: How many death penalties were carried out in each city?

```{r}
# Question: How many locations appear in each year?
library(DigitalMethodsData)
library(judges)
library(dplyr)
library(tidyr)
...
location_counts <- judges %>%
  group_by(Death.Year) %>%               # I grouped data by Death.Year
  summarise(Unique_Locations = n_distinct(Death.City))  # I counted unique Death.City for each year
...
```{r}
# Question: How many death penalties were carried out in each state?
library(DigitalMethodsData)
library(judges)
library(dplyr)
library(tidyr)
...
#colnames(judges)  #wanted to check my columns names

death_year_counts <- judges %>%  #I want to count how many deaths per state
  group_by(Death.State) %>%     # I groupped data by Death.Year
  summarise(Count = n())       # Count occurrences in each Death.Year

# View the result
print(death_year_counts)
...
```{r}
#Question: Which city had the most amenityfeatures? 
...
most_amenities_city <- gayguides %>%
  group_by(city) %>%                                   # I grouped by city
  summarise(feature_count = n_distinct(amenityfeatures)) %>%  # I  counted unique amenityfeatures for each city
  filter(feature_count == max(feature_count))          # I filtered cities with the max count

# View the result
print(most_amenities_city)
...
(@) Now write some code to address that question. Comment the code with notes that explain your thinking as you go. Use functions like select(), filter(), etc to pair down your dataset and reshape it to address your question. 
```{r}
#Questions: What amenities Did San Francisco have? 

colnames(gayguides)


san_francisco_amenities <- gayguides %>%
  filter(city == "San Francisco") %>%            # I filtered rows to just show San Francisco
  select(amenityfeatures) %>%                    # I picked out each amenityfeatures column
  distinct()                                     # Each different amenitty

# View the result
print(san_francisco_amenities)
...
#Question: How many locations were there in North Carolina in the year 1972?

filter_gayguides <- function(data, year, state) {
  #I filtered the dataset based on the provided NC (where I'm from) and 1972 (my birthday)
  filtered_data <- data %>%
    filter(Year == year, state == state) %>%  
    select(title, type, state, Year)           # I put in only required columns

  
}

result <- filter_gayguides(gayguides, year = 1972, state = "NC")
