...
```{r}
paste("R", "isn't so bad!" )
```

(@) Can you explain what happened in 2-3 sentences? 

> When I  use pastel()I can add two articles, comments, or textual items. To make a string sentence. I have split my string up into two parts connected by , to create a full sentence.
...
5 == 10  5 isn't equal to 10 - FALSE
3 < 4 3 is less than 4 - TRUE
5 != 10 5 isn't equal to 10 - TRUE
5 != 5 5 isn't equal to 5 - FALSE
3 == 4 | 3  3 is equal to 4 or 3 -TRUE
3 != 4 & 3 != 5 3 isn't equal to 4 or 5 - TRUE
...
my.numbers is actually the vector for 2, 4, 6, 8 multple by 2. When I run my numbers it doubles each of the numbers in vector, so it is == to 4, 8, 12, 16
...
(@) Explain in a few sentences, what happened in the code above?
 ...
 This took the third number of my vectors and multipled it by 2. So it is saying 6 times 2 which when ran is 12.


(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
my.grandkids  <-c ("Gaby", "Alaina", "Naomi", "Lucas", "Eleanor")
```
...
Column z is adding the other rows from left to right together, so it is coming up with the total of each row. It is basically tabulating the x and y columns.
>
...
This package provides sample datasets for the use of students in History 8510 at Clemson University. It is similar to the datasets and the histdata package.

References
References for each dataset included here is included on the documentation page.

Run `help(package="DigitalMethodsDaita")` in your console to pull up a list of datasets included in this package. 

almshouse_admissions -	Philadelphia Almshouse Admissions 1796-1803
BostonWomenVoters -	1920 Boston Women's Voter Register This data is derived from the Boston Archives and Records Management
charlestonManumissions -	Manumissions Data, Charleston District, South Carolina, 1776-1800
DigitalMethodsData - Datasets for Digital Methods II at Clemson University
gayguides -	Bob Damron's Address Books
judges-Federal judges in the United States of America
statepopulations -	Populations of US states and territories, 1790-2010
undergroundRR-	Underground RR Data The names/data of almost 1,000 freedom seekers who sought refuge with abolitionist William Still in 1850s.
...
Bobs Damron's Address Books from 1965 through 1985 in,
One data frame with 14 variables and 60698 obervations.
...
>I googled them, but also looked at other examples
...
gayguides is the dataset 
$ - lets you pull up a piece of the data, like a section. 
title - gives you the names of locations "Beach" or "B.A." in a row. 
[500] - gives you the 500th entry which is the Rest Stop.

>basically it's database$segment [numbered line]

(@) The Year variable contains the year (numeric) of each entry. Can you find the earliest year in this dataset? (Using code, don't cheat and use the documentation!) 1982
...
(@) How about the latest year?  1975
```{r}
gayguides$Year[60691]
...
CONGRATS! You've completed your first worksheet for Digital Methods II. That wasn't so bad, right? Not too bad 