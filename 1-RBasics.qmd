...
> When I  use pastel()I can add two articles, comments, or textual items. Like I was able to split up R, and the description isn't so bad! To make a string sentence.
...
3 != 4 & 3 != 5 3 isn't equal to 4 or 5 - TRU
...
my.numbers * 2
```
(@) Explain in a few sentences, what happened in the code above? my.numbers is actually the vector for 2, 4, 6, 8 multple by 2. When I run my numbers it doubles each of the numbers in vector, so it is == to 4, 8, 12, 16
...
my.numbers[3] * 2
```
(@) Explain in a few sentences, what happened in the code above? This took the third number of my vectors and multipled it by 2. So it is saying 6 times 2 which when ran is 12.
...
Column z is adding the other columns from left to right together, so it is coming up with the total of each row. 
>
...
For demonstration purposes we'll use the `gayguides` data here. Pull up the help documentation for this dataset. What does the documentation tell you about the scope of this dataset? Bobs Damron's Address Books from 1965 through 1985 in,
...
(@) How about the latest year? 1981
```{r}
gayguides$Year[length]
